...
import { ApiResponse, LoginCredentials, RegisterData, User, Tutor, TimeSlot, Booking, FilterOptions } from '@/types';
...
// The server will handle setting HttpOnly Secure cookies
...
export const authAPI = {
  ...
  register: async (data: RegisterData): Promise<ApiResponse<{ user: User } | null>> => {
    try {
  ...
  login: async (credentials: LoginCredentials): Promise<ApiResponse<{ user: User } | null>> => {
    try {
  ...
  // New method to get current user (for cookie auth)
  getCurrentUser: async (): Promise<ApiResponse<{ user: User } | null>> => {
    try {
...
export const tutorAPI = {
  searchTutors: async (filters: FilterOptions, page: number = 1, limit: number = 12): Promise<ApiResponse<{ tutors: Tutor[]; total: number; totalPages: number } | null>> => {
    try {
  ...
  getTutorById: async (id: string): Promise<ApiResponse<Tutor | null>> => {
    try {
...
export const bookingAPI = {
  createBooking: async (slotId: string): Promise<ApiResponse<{ booking: Booking; paymentUrl: string } | null>> => {
    try {
  ...
  getUserBookings: async (): Promise<ApiResponse<Booking[] | null>> => {
    try {
  ...
  processPayment: async (bookingId: string, paymentData: Record<string, unknown>): Promise<ApiResponse<{ success: boolean } | null>> => {
    try {
